...
{  
	...
	{ 
		...
		if(d->lss_transfer.mode==LSS_CONFIGURATION_MODE){
			if(m->data[1]>127 && m->data[1]!=0xFF){
				...
				// сохраняем новую node ID в EEPROM
				// при следующем включении устройства новое значение ноды
				// будет считано из EEPROM
			}
		}
		else{
	...
	case LSS_CONF_ACT_BIT_TIMING: /* Activate Bit Timing Parameters */
		...
		if(strcmp(d->lss_transfer.baudRate,"none")/*d->lss_transfer.baudRate!="none"*/){
